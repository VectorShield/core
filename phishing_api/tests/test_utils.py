...
def test_extract_eml_body_text():
    msg = EmailMessage()
    msg.set_content("This is a plain text email.")
    assert extract_eml_body(msg) == "This is a plain text email.\n"


def test_extract_eml_body_html():
    msg = EmailMessage()
    msg.add_alternative("<html><body><p>Hello, world!</p></body></html>", subtype="html")
    assert extract_eml_body(msg) == "Hello, world!\n"
...
def test_extract_email_features():
    ...
    features = extract_email_features(email_mock)
...
"""
    ...
    assert parsed_email["subject"] == "Test Email"
    assert parsed_email["sender"] == "sender@example.com"
    assert base64.b64decode(parsed_email["body"]).decode("utf-8") == "<html><body><p>This is a test email.</p></body></html>\n"
